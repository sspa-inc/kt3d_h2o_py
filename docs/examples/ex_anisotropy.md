...
This is because [`apply_transform()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/transform.py) works as follows:
...
2. Transform coordinates to model space using [`apply_transform()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/transform.py) with the same `angle_major` and `ratio`
...
> **Contract:** [`build_uk_model()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/kriging.py) checks `anisotropy_enabled`. When `False`, it sets `anisotropy_scaling=1.0` and `anisotropy_angle=0.0` in PyKrige.
...
4. **Clone the variogram** with `anisotropy_enabled=False` before passing to [`build_uk_model()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/kriging.py) to prevent double-application.
