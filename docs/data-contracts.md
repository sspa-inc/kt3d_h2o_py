...
**Loaded by:** [`load_observation_wells()`](../data.py)
...
> **Shapefile column name truncation:** Shapefiles truncate column names to 10 characters. [`load_line_features()`](../data.py) automatically checks both the full name and the first 10 characters when looking up `z_start_col` and `z_end_col`.
...
**Loaded by:** [`load_line_features()`](../data.py)
...
After loading, all sources are merged and cleaned by [`prepare_data()`](../data.py).
...
[`remove_duplicate_points()`](../data.py) is called with `min_dist = config["min_separation_distance"]`.
...
**Written by:** [`export_contours()`](../main.py)
...
**Written by:** [`export_aux_points()`](../main.py)
