...
This document describes every key in `config.json`. The file is loaded and validated by [`load_config()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/data.py) at startup. All four top-level sections (`data_sources`, `variogram`, `drift_terms`, `grid`) are **required**.
...
| `data_sources.linesink_river.strength_col` | string | No | `"resistance"` | Any numeric column name | Attribute column containing the hydraulic strength (resistance) of each segment. | Passed to [`compute_linesink_drift_matrix()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/AEM_drift.py) as `strength_col`. |
...
Defines the spatial correlation model. All three numeric parameters (`sill`, `range`, `nugget`) are **required** and validated by [`load_config()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/data.py) and [`variogram._validate_basic_parameters()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/variogram.py).
...
| Key Path | Type | Required | Default | Allowed Values | Description | Interactions |
|---|---|---|---|---|---|---|
| `variogram.anisotropy.enabled` | boolean | No | `false` | `true`, `false` | Whether to apply anisotropy pre-transformation. | When `true`, `ratio` and `angle_major` are validated by [`_validate_anisotropy()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/variogram.py). PyKrige's internal anisotropy is disabled to avoid double-application. |
...
| Key Path | Type | Required | Default | Allowed Values | Description | Interactions |
|---|---|---|---|---|---|---|
| `min_separation_distance` | float | No | — | `>= 0` | Minimum allowed distance (in CRS units) between any two data points. Points closer than this threshold are deduplicated by [`remove_duplicate_points()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/data.py), retaining the first occurrence. | Applied after merging observation wells and control points. Set to `0` or omit to disable deduplication. |
...
> **Critical contract:** When `use_linesink` is `true`, the scaling factors computed during training are stored as `trained_scaling_factors` and **must** be passed to [`predict_on_grid()`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/kriging.py) via the `scaling_factors` parameter. Failure to do so will cause the prediction-time AEM potential to be rescaled independently, producing inconsistent drift columns between training and prediction.

> **Source references:** [`main.py-436`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/main.py), [`data.py-128`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/data.py), [`variogram.py-52`](https://github.com/sspa-inc/kt3d_h2o_py/blob/main/variogram.py)
